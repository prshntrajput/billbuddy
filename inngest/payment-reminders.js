...
export const paymentReminders = inngest.createFunction(
  {
    ...
    triggers: [{ cron: "0 10 * * *" }], // ✅ moved inside first arg as array
  },
  async ({ step }) => {                  // ✅ handler is now second arg
    ...
    const results = await step.run("send-emails", async () => {
      return Promise.all(
        users.map(async (u) => {
          ...
          const html = `
            <h2>BillBuddy – Payment Reminder</h2>
          ...
          try {
            await convex.action(api.email.sendEmail, {
              to: u.email,
              subject: "You have pending payments on BillBuddy",
              html,
