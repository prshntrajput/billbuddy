...
export function ExpenseForm({ type = "individual", onSuccess }) {
  ...
  // ✅ useWatch instead of watch() — React Compiler compatible
  const amountValue = useWatch({ control, name: "amount" });
  const paidByUserId = useWatch({ control, name: "paidByUserId" });

  // ✅ setState during render — no useEffect needed for this
  // React detects the state change mid-render, discards it, re-renders with new state
  const [lastUserId, setLastUserId] = useState(null);
  if (currentUser?._id && lastUserId !== currentUser._id && participants.length === 0) {
    setLastUserId(currentUser._id);
    setParticipants([
      {
        id: currentUser._id,
  ...
  // ✅ setValue updates RHF (an external system) — useEffect IS correct here
  useEffect(() => {
    if (currentUser?._id) {
      setValue("paidByUserId", currentUser._id);
    }
  }, [currentUser?._id, setValue]);

  // ✅ Stable reference for onSplitsChange — prevents SplitSelector re-renders
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (onSuccess) onSuccess(type === "individual" ? otherParticipant?.id : groupId);
    } catch (error) {
